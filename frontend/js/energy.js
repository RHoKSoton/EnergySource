var energy = {
	...
	init: function() {
		...
		for (var component in data.components) {
			for (var i = 0, i_len = data.components[component].length; i < i_len; i++) {
				...
				components_list.append("<tr><td style='height:16px'><input name='component' data-comp_name='" + mf_name + "' id='" + mf_name + "' type='radio' /></td><td class='style3'><a class='component_link' href=''>" + mf_name + "(" + mf.Part + ") </a></td><td class='style4'><a class='score_link'><img class='score_light' align='right' alt='' src='images/Button-Blank-Red-icon.png' /></a></td></tr>");
			}
		}
		$(".score_link").hide();
	...
	city_clicked: function(city_name, country_name) {
    console.log("Clicked: "+city_name+", "+country_name);
		energy.clicked_country = country_name;
